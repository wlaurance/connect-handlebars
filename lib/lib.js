...
var lib = {
    ...
    generateTemplateRegExp: function(exts) {
        ...
        if ( typeof exts == 'string' ) {
          ...
          exts_re = new RegExp("\\" + exts.join("|") + "$");
            ...
            throw new Error('Express-Ember-Handlebars option exts only accepts string or array object');
        }
        return exts_re;
    },
    ...
    _processFile: function(options, path, template, root, self, output, stat) {
        ...
        options.timestamps[key] = data.toString().replace(/\n/g,'');
        options.timestamps[key] = options.timestamps[key].replace(/'/g,"\\'");
        output.push('Ember.TEMPLATES[\'' + key + '\'] = Ember.Handlebars.compile(\'' + options.timestamps[key] + '\');\n');
    }
};
